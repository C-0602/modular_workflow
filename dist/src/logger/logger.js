...
let WinstonLogger = class WinstonLogger {
    ...
    constructor(config) {
        this.config = config;
        const logFolder = this.config.get('logging.folder') || 'logs';
        const level = this.config.get('logging.level') || 'info';
        const rotation = this.config.get('logging.rotation') || { maxFiles: '14d', maxSize: '20m' };
